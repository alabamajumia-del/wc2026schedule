...
const dataDownloadFiles = [
  {
    ...
    includes: "104 rows",
    fileSize: "31 KB",
    version: "wc26schedule dataset v1",
    updated: "May 24, 2026",
    source: "Generated from structured wc26schedule match data based on FIFA schedule references.",
    actionLabel: "Download CSV data"
  },
  {
    ...
    includes: "8 sheets",
    fileSize: "355 KB",
    version: "wc26schedule workbook v1",
    updated: "May 24, 2026",
    source: "Generated by wc26schedule for spreadsheet planning and source-note review.",
...
const pdfDownloadFiles = [
  {
    ...
    fileSize: "107 KB",
    version: "wc26schedule printable v1",
    updated: "May 24, 2026",
    source: "Generated by wc26schedule from the structured match dataset.",
    actionLabel: "Download printable PDF"
  },
  {
    ...
    fileSize: "40 KB",
    version: "wc26schedule poster v1",
    updated: "May 24, 2026",
    source: "Generated by wc26schedule for quick visual scanning.",
    actionLabel: "Download overview PDF"
  },
  {
    ...
    fileSize: "36 KB",
    version: "wc26schedule stage v1",
    updated: "May 24, 2026",
    source: "Generated by wc26schedule for group-stage and knockout-stage planning.",
    actionLabel: "Download stage PDF"
  },
  {
    label: "Bracket PDF",
    ...
    includes: "2 pages",
    fileSize: "938 KB",
    version: "wc26schedule bracket reference",
    updated: "May 23, 2026",
    source: "Added as a bracket-focused planning file for knockout-route users.",
    actionLabel: "Download bracket PDF"
  }
];
...
const visualDownloadFiles = [
  {
    ...
    fileSize: "1.9 MB",
    version: "wc26schedule overview image",
    updated: "May 23, 2026",
    source: "Generated by wc26schedule as a visual planning image.",
    actionLabel: "Download overview image"
  }
];
...
const renderTrustStrip = (corePhrase, mode = "download") => `<div class="download-trust-strip" aria-label="${attr(corePhrase)} trust and compliance details">
  <article>
    <span>Updated</span>
    <strong>May 24, 2026</strong>
    <p>Generated from the current wc26schedule match dataset.</p>
  </article>
  <article>
    <span>Source basis</span>
    <strong>Official schedule references</strong>
    <p>Use the files for planning, then confirm paid or time-sensitive decisions with official sources.</p>
  </article>
  <article>
    <span>Independence</span>
    <strong>Independent fan planner</strong>
    <p>wc26schedule is not affiliated with FIFA and does not sell tickets or collect payments.</p>
  </article>
  <article>
    <span>Best next step</span>
    <strong>${mode === "excel" ? "Filter, then verify" : "Download, then verify"}</strong>
    <p>${mode === "excel" ? "Keep a clean source copy before adding personal notes." : "Refresh saved copies before travel, tickets or public sharing."}</p>
  </article>
</div>`;

const renderDownloadDecisionGrid = (corePhrase, decisions) => `<div class="download-decision-grid">
  ${decisions
    .map(
      (item) => `<article class="download-decision-card">
    <span>${esc(item.task)}</span>
    <strong>${esc(item.recommendation)}</strong>
    <p>${esc(item.copy)}</p>
    <a href="${attr(item.href)}"${item.download ? " download" : ""}>${esc(item.action)}</a>
  </article>`
    )
    .join("")}
</div>`;
...
const renderExcelDownloadSelector = (page = {}) => {
  const excelFiles = [
    {
      ...
      updated: "May 24, 2026",
      version: "wc26schedule workbook v1",
      source: "Generated by wc26schedule from structured match data.",
      actionLabel: "Download Excel workbook"
    },
    {
      format: "CSV",
      ...
      updated: "May 24, 2026",
      version: "wc26schedule dataset v1",
      source: "Generated by wc26schedule for imports and custom planners.",
      actionLabel: "Download CSV data"
    }
  ];
  const decisions = [
    {
      task: "I need to filter teams",
      recommendation: "Use the Excel workbook",
      copy: "Open the By Team sheet, then keep a clean source copy before adding travel or watch-party notes.",
      href: "/downloads/world-cup-2026-schedule.xls",
      action: "Download workbook",
      download: true
    },
    {
      task: "I need Google Sheets",
      recommendation: "Use the CSV data",
      copy: "Import the raw rows when you want your own formulas, dashboards, Airtable views or database fields.",
      href: "/downloads/world-cup-2026-schedule.csv",
      action: "Download CSV",
      download: true
    },
    {
      task: "I need a printed copy",
      recommendation: "Use the PDF library",
      copy: "Filter in Excel first if needed, then move to the PDF page once the match list is ready to share.",
      href: "/world-cup-2026-schedule-pdf/",
      action: "Open PDF options"
    }
  ];
  ...
  ${renderTrustStrip("World Cup 2026 Schedule Excel", "excel")}
  ${renderDownloadDecisionGrid("World Cup 2026 Schedule Excel", decisions)}
  <div class="excel-file-grid">
    ${excelFiles
      ...
      <dl>
        ...
        <div><dt>Updated</dt><dd>${esc(file.updated)}</dd></div>
        <div><dt>File</dt><dd>${esc(`${file.fileSize} - ${file.version}`)}</dd></div>
      </dl>
      <ul>
        ${file.includes.map((item) => `<li>${esc(item)}</li>`).join("")}
      </ul>
      <p class="download-source-note">${esc(file.source)} Not affiliated with FIFA; verify official details before travel, tickets or publishing.</p>
      <a class="button" href="${attr(file.href)}" download>${esc(file.actionLabel)}</a>
    </article>`
      )
      .join("")}
  </div>
...
const downloadPanel = (page = {}) => {
  ...
  const pdfDecisions = [
    {
      task: "I want to print",
      recommendation: "Printable PDF",
      copy: "Best when you need match numbers, dates, teams, host cities and stadiums in one offline file.",
      href: "/downloads/world-cup-2026-schedule.pdf",
      action: "Download printable PDF",
      download: true
    },
    {
      task: "I want a quick scan",
      recommendation: "Overview poster PDF",
      copy: "Best for group chats, planning boards and fast visual checks by date and host city.",
      href: "/downloads/world-cup-2026-schedule-overview.pdf",
      action: "Download overview PDF",
      download: true
    },
    {
      task: "I want stage planning",
      recommendation: "Stage overview PDF",
      copy: "Best when group-stage fixtures and knockout-stage placeholders need separate pages.",
      href: "/downloads/world-cup-2026-stage-overview.pdf",
      action: "Download stage PDF",
      download: true
    },
    {
      task: "I need filtering first",
      recommendation: "Excel or CSV",
      copy: "Use the spreadsheet page when you need team, city, date or stage filters before saving a final file.",
      href: "/world-cup-2026-schedule-excel/",
      action: "Open Excel options"
    }
  ];
  const card = (file) => `<a class="download-card" href="${attr(file.href)}" download>
      ...
      <dl>
        ...
        <div><dt>Updated</dt><dd>${esc(file.updated ?? updated)}</dd></div>
        <div><dt>File</dt><dd>${esc([file.fileSize, file.version].filter(Boolean).join(" - "))}</dd></div>
      </dl>
      <em>${esc(file.actionLabel ?? `Download ${file.format}`)}</em>
    </a>`;
  ...
  ${isPdfPage ? renderTrustStrip("World Cup 2026 Schedule PDF", "pdf") : ""}
  ${isPdfPage ? renderDownloadDecisionGrid("World Cup 2026 Schedule PDF", pdfDecisions) : ""}
